...
class companionUI:
    ...
    def setupSendFrame(self):
        ...
        sendStartSending = tkinter.Button(sendFrame, text="start", command=self.grids).grid(row=3, column=4, sticky=tkinter.EW)
    ...
    def setuprecieveFrame(self):
        ...
        #revieve and save
        recieveSelectFile = tkinter.Button(recieveFrame, text="selectTx", command=self.selectFile).grid(row=2, column=4)
